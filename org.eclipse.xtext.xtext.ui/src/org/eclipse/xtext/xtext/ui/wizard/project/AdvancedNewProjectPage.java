...
public class AdvancedNewProjectPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		Group(control, (group) -> {
			...
			preferredBuildSystem = DropDown(group, (combo) -> {
				...
				combo.setItems(FluentIterable.from(BuildSystem.values()).transform((bs) -> bs.toString()).toArray(String.class));
			});
		});
		...
		Group(control, (group) -> {
			...
			createLanguageServer = DropDown(group, (combo) -> {
				...
				combo.setItems(FluentIterable.from(LanguageServer.values()).transform((ls) -> ls.toString()).toArray(String.class));
				InfoDecoration(group, Messages.AdvancedNewProjectPage_languageServer_description);
			});
		});
		...
		Group(control, (group) -> {
			...
			sourceLayout = DropDown(group, (combo) -> {
				combo.setEnabled(true);
				combo.setItems(FluentIterable.from(SourceLayout.values()).transform((sl) -> sl.toString()).toArray(String.class));
			});
		});
	...
	public void checkWidgets(SelectionEvent e) {
		...
		if (createUiProject.getSelection() && !isSelected(sourceLayout, SourceLayout.PLAIN)) {
			if (createUiProject == source) {
				String message = createUiProject.getText() + " requires " + SourceLayout.PLAIN + " source layout. " //
						+ "Please select '" + SourceLayout.PLAIN + "' source layout.";
				...
				String message = SourceLayout.MAVEN + " source layout is not supported by the " + createUiProject.getText() + " project. " //
						+ "Please deselect '" + createUiProject.getText() + "'.";
				reportIssue(ERROR, message, () -> {
					createUiProject.setSelection(false);
				});
			}
		}
		...
		if (isSelected(preferredBuildSystem, BuildSystem.NONE) && isSelected(sourceLayout, SourceLayout.MAVEN)) {
			if (preferredBuildSystem == source) {
				...
				String message = "Maven/Gradle source layout is only supported when using Maven or Gradle build system. " //
						+ "You need to choose Maven or Gradle build system. Select Gradle build.";
				reportIssue(ERROR, message, () -> {
					select(preferredBuildSystem, BuildSystem.GRADLE);
				});
			}
		}
		...
		if (createWebProject.getSelection() && isSelected(preferredBuildSystem, BuildSystem.NONE)) {
			if (preferredBuildSystem == source) {
				String message = "The " + createWebProject.getText() + " project can not be build without a build system. " //
						+ "Please deselect '" + createWebProject.getText() + "'.";
				reportIssue(ERROR, message, () -> {
					createWebProject.setSelection(false);
				});
			} else {
				String message = "To build the '" + createWebProject.getText() + "' project can not be build without a build system. " //
						+ "Please deselect '" + createWebProject.getText() + "'.";
				reportIssue(ERROR, message, () -> {
					select(preferredBuildSystem, BuildSystem.GRADLE);
				});
		...
		boolean isSelected = Iterables.any(uiButtons, uiButton -> {
			return uiButton.getSelection();
		});
		if (isSelected) {
			createIdeProject.setEnabled(false);
		}
		...
		if (autoSelectIdeProject) {
			...
			String message = "'" + createSDKProject.getText() + "' was automatically selected as option '" + ((Button) source).getText()
					+ "' requires it.";
			reportIssue(INFORMATION, message);
		}
	}

	protected void select(Combo combo, Enum<?> e) {
		String[] items = combo.getItems();
		for (int i = 0; i < items.length; i++) {
			if (e.toString().equals(items[i])) {
				combo.select(i);
				break;
			}
		}
	...
	protected boolean isSelected(Combo combo, Enum<?> enu) {
		return Objects.equal(enu.toString(), combo.getText());
	}
	...
	protected boolean isBundleResolved(String bundleId) {
		...
		Optional<Bundle> b = Arrays.stream(bundles).filter(bundle -> bundleId.equals(bundle.getSymbolicName())).findFirst();
		return b.isPresent() && (b.get().getState() & (Bundle.RESOLVED | Bundle.STARTING) | Bundle.ACTIVE) != 0;
	}
